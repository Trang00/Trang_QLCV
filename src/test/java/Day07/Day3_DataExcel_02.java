...
public class Day3_DataExcel_02 {
    ...
    // Method 	getDataFromExcel is defined in class Util
    	String[][] testData = Util_Day7.getDataFromExcel(Util_Day7.FILE_PATH,
    			Util_Day7.SHEET_NAME, Util_Day7.TABLE_NAME);
	...
	for (int i = 0; i < testData.length; i++) {
	    ...
	    try{ 
			...
			alt.accept();
			if (actualBoxtitle.contains(Util_Day7.EXPECT_ERROR)) { // Compare Error Text with Expected Error Value
			...
			if (actualTitle.contains(Util_Day7.EXPECT_TITLE)) {
			    System.out.println("Test case SS[" + i + "]: Passed");
			} else {
			    System.out.println("Test case SS[" + i + "]: Failed");
			}
